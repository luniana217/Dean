...
def main():
  try :
   ...
   if res.status_code == 200:
       ...
       tab1, tab2, tab3 = st.tabs(["HTML 데이터","JSON데이터","DataFrame"])
       with tab1:
          st.html(table)
       with tab2:
          json_string = json.dumps(episode, ensure_ascii=False, indent=2)
          st.download_button(
             label="JSON 다운로드",
             data=json_string,
       ...
       with tab3:
          st.dataframe(
             pd.DataFrame(episode).drop(columns=['season']),
