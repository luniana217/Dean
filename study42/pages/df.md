...
from requests import get #문서를 가져오기 위한 영역
import json
import pandas as pd
...
if 'link_index' not in st.session_state:
	...
	#리액트로 치면, 이걸 안하면 함수끼리 데이터를 주고 받을수 없다
...
genre_codes = ["GN0100","GN0200","GN0300","GN0400","GN0500","GN0600","GN0700","GN0800"]
...
def main():
  try:
    ...
    #time.sleep(2)  #자료 수집을 위한 콛 ㅡ넣어주는곳
    ...
    #   print(values)
       saveMany(None, sql2, values) 
        ...
        # st.html(res.text) #(res.text.replace("\n","").strip()) 필요한 자료를 뽑아오기 위한거
...
if selected:
  ...
  st.session_state.link_index = options.index(selected) #인덱스를 넣기위해 옵션을 넣어줌
  if st.button(f"'{options[st.session_state.link_index]}' 수집"):  #버튼은 기본적으로 true or false ,
    if main() == 0:
      st.text("수집된 데이터가 없습니다.")
#select 박스가 있어야 html에서 쓸수 있다
