...
class Node(BaseModel):
  id: str  # 노드 ID (예: N0)  "N0", "N1" 같은 식별자
  label: str  # 노드 타입 (예: "인간")
  properties: Optional[Dict[str, PropertyValue]] = None  # 속성 딕셔너리  {"name": "탄지로"} 등

class Relationship(BaseModel):
  type: str  #  figths,train 같은 관계 유형
  start_node_id: str  #관계 시작 노드 ID
  end_node_id: str  #관계 끝 노드 ID
  properties: Optional[Dict[str, PropertyValue]] = None  # 관계 속성

class GraphResponse(BaseModel): #LLM이 return 할 최종구조. Pydantic이 타입검증을 자동으로 해줌
