...
def read_root(model:LoginModel):
    sql = f"select `no`,`name` from edu.user where `email` = '{model.email}' and `password` ={model.password}"
    data = findOne(sql)
    if data:
...
def read_root(model:LoginModel):
   ...
   data=findOne(sql)
   if data:
      token = token(data['no'],data['name'])
      sql = f"INSERT into login (`id`,`userNo`,`password`) value({id}, {data['no']},{data['name']})"
   if save(sql): 
      return{"status" : True, "model":model, "token": id}   
